...
pub fn solve() -> (Option<i64>, Option<i64>) {
    ...
    let numbers = numbers.split(',').map(|x| x.parse().unwrap()).collect::<Vec<i64>>();
...
fn solve2(numbers: Vec<i64>) -> i64 {
    let mut seen_numbers: HashMap<i64, i64> = numbers.iter().enumerate().map(|(v, &k)| (k, v as i64 + 1)).collect();
